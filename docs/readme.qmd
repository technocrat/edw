...
One operation, splitting the imported `ndvi` object into a directory of `csv` files is best done using a `rust` program, which requires the installation of the [`rust` toolchain](https://www.rust-lang.org/learn/get-started).

```
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh
```

If the installation is successful, install the `rust` program that will process the `csv` files.
...
If there is trouble with the installation, please let me know and I will bind an alternative.
...
3. Extract ice/snow points using the `find_frozen` function using the raw `csv` data from step 2
4. Scale the raw `csv` data using the `stl.R` script
5. Interpolate missing values in the scaled data using the `interpolate` function 
6. Smooth the interpolated data with a filtered running mean using the `make_splines` function
7. Smooth splines on smoothed data using the `make_smooth` function
8. Take derivatives of smooth splines using the `make_sss` function
